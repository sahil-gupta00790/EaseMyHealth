...
func (app *application) generateJWT(user *data.User) (string, time.Time, error) {
	// Set token expiration time (e.g., 24 hours from now)
	expiry := time.Now().Add(15 * time.Hour)

	// Create a new token object
	claims := jwt.MapClaims{
		"user_id": user.UserID.String(), // Convert UUID to string
	...
	// Generate signed token string using your secret key
